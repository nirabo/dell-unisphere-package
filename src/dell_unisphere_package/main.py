...
class BasicSystemInfo(BaseModel):
    ...
    softwareFullVersion: str = "Unity 5.3.0.0 (Release, Build 120, 2023-03-18 19:02:01, 5.3.0.0.5.120)"
    apiVersion: str = "13.0"
...
async def verify_required_headers(request: Request, call_next):
    ...
    if request.url.path.startswith("/api") or request.url.path.startswith("/upload"):
        if "X-EMC-REST-CLIENT" not in request.headers or request.headers["X-EMC-REST-CLIENT"] != "true":
            return JSONResponse(
                status_code=401,
...
def format_response(data, request: Request, base_url="/api", instance_type=None, instance_id=None):
...
async def get_basic_system_info(request: Request):
    ...
    return format_response([system_info.dict()], request, instance_type="basicSystemInfo")

@app.get("/api/types/loginSessionInfo/instances")
async def get_login_session_info(request: Request, response: Response, current_user = Depends(get_current_user)):
    if not current_user:
    ...
    return format_response([session_info.dict()], request, instance_type="loginSessionInfo")
...
async def get_candidate_software_versions(
    ...
    return format_response(candidates, request, instance_type="candidateSoftwareVersion")
...
async def get_upgrade_sessions(
    ...
    if fields:
        ...
        for session in upgrade_sessions.values():
            filtered_session = {field: session.get(field) for field in field_list if field in session}
...
async def create_upgrade_session(
    ...
    if candidate_id not in candidate_software_versions and candidate_id != "candidate_1":
